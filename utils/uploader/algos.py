...
def heap_sort(raw_data: list[int]):
    data = raw_data.copy()
...
algos.heap = Algorithm('Пирамидальная', 'Выбором', 'O(n log n)',
                       19, code_heap, description_heap)
...
def merge_sort(raw_data: list[int]):
...
""")
description_merge = (
    "Сортировка основана на парадигме «разделяй и властвуй». Разделим массив по"
    "полам, рекурсивно отсортируем части, после чего выполним процедуру слияния"
    ": поддерживаем два указателя, один на текущий элемент первой части, второй"
    " – на текущий элемент второй части. Из этих двух элементов выбираем минима"
    "льный, вставляем в ответ и сдвигаем указатель, соответствующий минимуму.\n"
    "\n"

    "Слияние работает за O(n), уровней всего logn, поэтому асимптотика O(nlogn)"
    ". Эффективно заранее создать временный массив и передать его в качестве ар"
    "гумента функции. Эта сортировка рекурсивна, как и быстрая, а потому возмож"
    "ен переход на квадратичную при небольшом числе элементов."
)
algos.merge = Algorithm('Слиянием', 'Выбором', 'O(n log n)',
                        21, code_merge, description_merge)
...
description_stl_sort = (
    "Представленная в STL функция sort() являет собой реализацию Introsort — ал"
    "горитм сортировки, предложенный Дэвидом Мюссером в 1997 году. Сначала выпо"
    "лняется сортировка, аналогичная алгоритму QuickSort, после чего переключае"
    "тся на HeapSort (пирамидальная сортировка), а в самом конце, когда остаютс"
    "я небольшие интервалы (32 и менее элементов), они сортируются сортировкой "
    "вставками. Компиляторы также предоставляют различные оптимизации, и вышепе"
    "речисленный алгоритм актуален для наиболее часто используемого компилятора"
    " GCC. LLVM же реализует весьма сложный алгоритм с множеством оптимизаций в"
    " зависимости от размеров сортируемых интервалов и того, являются ли сортир"
    "уемые элементы тривиально копируемыми и тривиально конструируемыми. Функци"
    "я принимает аргументами итераторы произвольного доступа (first, last) и вы"
    "полняет сортировку на месте, то есть, модифицирует данный контейнер."
)
algos.stl_sort = Algorithm(
    'sort() (C++, STL)', 'Выбором', 'O(n*log(n))', 1, code_stl_sort, description_stl_sort)

# ? STL - stable_sort()
code_stl_stable_sort = (
    """void _stable_sort(vector<int> vec) {
    std::stable_sort(vec.begin(), vec.end());
}""")
description_stl_stable_sort = (
    "Согласно названию, данный алгоритм является стабильным, то есть, он не мен"
    "яет местами эквивалентные элементы при сортировке. Фактически является опт"
    "имизированной и доработанной реализацией MergeSort (сортировка слиянием). "
    "Если памяти достаточно, можно использовать стандартный MergeSort, но это с"
    "ложный алгоритм, и в обычных условиях он будет работать не очень быстро. С"
    "оответственно, обычно используется MergeSort без дополнительной памяти, ас"
    "имптотика которого n*(log(n))² сравнений. Обе реализации, когда дело доход"
    "ит до небольших интервалов, используют сортировку вставками, которая также"
    " стабильная и не требует дополнительной памяти, но её сложность — O(n²), ч"
    "то, впрочем, не играет большой роли на маленьких интервалах, где она и исп"
    "ользуется."
)
algos.stl_stable_sort = Algorithm(
    'stable_sort() (C++, STL)', 'Выбором', 'O(n*log(n)) / O(n*(log(n))²)',
    1, code_stl_stable_sort, description_stl_stable_sort)
...
description_stl_sort_heap = (
    "std::make_heap() и std::sort_heap() — это алгоритмы по работе с кучей (max"
    " heap), включая сортировку. Это единственный способ сортировки, алгоритм д"
    "ля которого указан явно. Сортировка реализована, как и в обычной heapsort "
    "(пирамидальной сортировке)."
)
algos.stl_sort_heap = Algorithm(
    'sort_heap() (C++, STL)', 'Выбором', 'O(n*log(n))',
    1, code_stl_sort_heap, description_stl_sort_heap)
